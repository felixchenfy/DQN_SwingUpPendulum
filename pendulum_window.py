...
class myWindow(object):
    ...
    def init_window(self):
        ...
        self.window.geometry('{0}x{1}+500+150'.format(WINDOW_COLS, WINDOW_ROWS))
        self.canvas = tk.Canvas(self.window, bg='white', height=WINDOW_ROWS, width=WINDOW_COLS)
    ...
    def bind_keys(self):
        self.canvas.bind("<KeyPress>", self.event_keydown)
        self.canvas.pack()
    ...
    def display_text(self):
        ...
        str1="User input:\n"
        str1+="A,D: rotate link1\n"
        str1+="W,S: rotate link2\n"
        str1+="J,K: apply torque to link1\n"
        str1+="Q: reset game\n"
        # str1+="E/R: enable/disable simulation\n" # the sim engine is in "pendulum_env"
        ...
        self.canvas.create_text(xpos, ypos, text=str1, font=("Purisa", fontsize), anchor=tk.SW) 
    ...
    def restart(self):
        ...
        base_link = self.canvas.create_oval(x0, y0, x1, y1, fill=BASE_LINK_COLOR)
        ...
        x1 = link1_end_x
        y1 = link1_end_y
        link1 = self.canvas.create_line(x0, y0, x1, y1, width=LINK1_WIDTH, fill=LINK1_COLOR)

        # 3. link2
        link2_end_x = link1_end_x+LINK2_LENGTH*math.cos(link1_angle+link2_angle)
        link2_end_y = link1_end_y+LINK2_LENGTH*math.sin(link1_angle+link2_angle)
        x0 = link1_end_x
        y0 = link1_end_y
        x1 = link2_end_x
        y1 = link2_end_y
        link2 = self.canvas.create_line(x0, y0, x1, y1, width=LINK2_WIDTH, fill=LINK2_COLOR)
        ...
        y0 = 0
        y1 = self.WINDOW_ROWS
        wall_right = self.canvas.create_rectangle(x0, y0, x1, y1, fill=WALL_COLOR)
        ...
        y1 = self.WINDOW_ROWS
        wall_down = self.canvas.create_rectangle(x0, y0, x1, y1, fill=WALL_COLOR)
        ...
        y0 = 0
        y1 = self.WINDOW_ROWS
        wall_left = self.canvas.create_rectangle(x0, y0, x1, y1, fill=WALL_COLOR)
    ...
    def update_ball(self):
        ...
        newpose=[x0-BALL_RADIUS, y0-BALL_RADIUS,x0+BALL_RADIUS,y0+BALL_RADIUS]
        self.canvas.coords(self.ball, *newpose)

    def event_keydown(self, e):
        c=e.char.lower()
